...
function kap(t, fun,     u) --> t; map function `fun`(k,v) over list (skip nil results)
  u={}; for k,v in pairs(t) do v,k=fun(k,v); u[k or (1+#u)]=v; end;
  --[[
...
function o(t,isKeys,     fun) --> s; convert `t` to a string. sort named keys.
  ...
  fun= function(k,v) print(tostring(k):find"^_") if not tostring(k):find"^_" then return fmt(":%s %s",o(k),o(v)) end end
  -- #t > 0 : 0 > 0 : False
  -- isKeys : nil -> false, not isKeys: True; I guess False and True -> False so -> sort(kap(t,fun))
  -- print("{"..table.concat(sort(kap(t,fun)), " ").."}") ==== {:dump false :go the :help false :seed 937162211}
...
help:false }
--]]
  if options.help then print(help) else --THIS else condition is executed for THE
    for what,fun in pairs(funs) do
      -- funs = egs
      -- what = the; num sym rand
      -- options.go = the
      if options.go=="all" or what==options.go then
        -- saved has everything as options
         for k,v in pairs(saved) do options[k]=v end
         Seed = options.seed --937162211
...
local function eg(key,str, fun) --> nil; register an example.
  egs[key]=fun -- egs[the] = function oo(the) end
  -- fmt("  -g  %s\t%s\n",key,str) :   -g  the       show settings <-- the ACTIONS
  help=help..fmt("  -g  %s\t%s\n",key,str) end
